...
class GraphNetwork(object):
    def __init__(self, batch_size=16, block_size=500):
        self.data_loader = Dataloader(batch_size=batch_size, block_size=block_size)

    def build_model(self):
        f_data = keras.layers.Input(shape=(None, self.data_loader.max_len), name='facebook_data')
        f_edge = keras.layers.Input(shape=(None, None), name='facebook_edge')

        t_data = keras.layers.Input(shape=(None, self.data_loader.max_len), name='twitter_data')
